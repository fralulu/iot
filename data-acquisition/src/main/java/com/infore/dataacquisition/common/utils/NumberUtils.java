...
public class NumberUtils {
	...
	public static Integer getMax( Integer[] numbers ) {
		...
		for ( int i = 0; i < numbers.length; i++ ) {
			...
			//如果max为null，则将value赋值给它
			max = max == null ? value : null;
	...
	/**
	 * <p>计算两个数字相除的结果并四舍五入返回，如果除数为0，则返回0</p>
	 * <pre>
	 * 	divided( 9L, 0L, 0 ) = 0
	 * 	divided( 9L, 10L, -1 ) = 0
	 * </pre>
	 ...
	 * @return Double		计算的结果
	 */
	public static Double divided( double dividend, double divisor ) {
		return NumberUtils.divided(dividend, divisor, 2);
	}
	...
	public static String setDecimal(Double value,int decimal){
		if(value==null){
			return 0.0+"";
		}
		String string=null;
		...
		string = new DecimalFormat(dec).format(value);
		return string;
	}
